...
class EarllmIntegrationTests(unittest.TestCase):
	...
	def test_budget_audit_events_are_scoped_by_wallet_and_entry(self):
		first_wallet = add_wallet(self.database_path, {"name": "First", "course": "HDL"})
		second_wallet = add_wallet(self.database_path, {"name": "Second", "course": "LCD"})
		first_entry = add_budget_entry(self.database_path, {"title": "First deposit", "type": "deposit", "amount": 100, "reason": "Funds", "wallet_id": first_wallet["id"]})
		second_entry = add_budget_entry(self.database_path, {"title": "Second deposit", "type": "deposit", "amount": 200, "reason": "Funds", "wallet_id": second_wallet["id"]})
		update_budget_entry(self.database_path, first_entry["id"], {"status": "spent", "actor": "tester"})

		first_events = list_budget_audit_events(self.database_path, wallet_id=first_wallet["id"])
		second_events = list_budget_audit_events(self.database_path, wallet_id=second_wallet["id"])
		entry_events = list_budget_audit_events(self.database_path, entry_id=first_entry["id"])
		self.assertTrue(first_events)
		self.assertTrue(second_events)
		self.assertTrue(all(event["wallet_id"] == first_wallet["id"] for event in first_events))
		self.assertTrue(all(event["wallet_id"] == second_wallet["id"] for event in second_events))
		self.assertTrue(all(event["entry_id"] == first_entry["id"] for event in entry_events))
		self.assertNotIn(second_entry["id"], {event["entry_id"] for event in first_events})

	def test_rich_text_preserves_safe_preformatted_content(self):
		value = sanitize_rich_text('<pre>  1\n  2</pre><script>alert(1)</script>')
		self.assertIn('<pre>  1\n  2</pre>', value)
