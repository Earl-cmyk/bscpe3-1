	...
	with get_connection(database_path) as connection:
		...
		lesson_contents = {
			...
			"compound_interest_lesson": (
				"Compound interest uses the formula F = P(1 + i)^n, where i is the interest rate per compounding "
				"period and n is the total number of periods. This formula grows money exponentially over time, "
				"and the principal may be solved by rearranging the equation when the future value is known."
			),
			"discount_lesson": (
				"Discounting is the reverse of compounding. The discount amount is D = F d t, where F is the face or "
				"future value, d is the discount rate, and t is time. The present value is P = F - D or P = F(1 - d t)."
			),
			"equation_of_value_lesson": (
				"Equation of value compares all amounts at a common focal date, then balances the present or future "
				"equivalent values. The key idea is that money has time value, so each obligation and payment must be "
				"moved to the same time before the equation is solved."
			),
			"nominal_rate_lesson": (
				"A nominal rate is the annual rate quoted before adjusting for compounding frequency. The relationship "
				"is j = n1 x i, where n1 is the number of compounding periods each year and i is the rate per period."
			),
			"effective_rate_lesson": (
				"The effective annual rate converts a nominal rate into the actual annual growth rate. The expression "
				"i_e = (1 + j / n1)^n1 - 1 tells how much interest is truly earned over one year."
			),
			"continuous_compounding_lesson": (
				"Continuous compounding assumes interest is added continuously, giving the model F = P e^(j t). "
				"This is the limit of compounding as the compounding interval becomes infinitely small."
			),
		}
		for source_key, content in lesson_contents.items():
			title = source_key.replace("_lesson", "").replace("_", " ").title()
			if source_key == "simple_interest_lesson":
				title = "Simple Interest Lesson"
			elif source_key == "compound_interest_lesson":
				title = "Compound Interest Lesson"
			elif source_key == "discount_lesson":
				title = "Discount Lesson"
			elif source_key == "equation_of_value_lesson":
				title = "Equation of Value Lesson"
			elif source_key == "nominal_rate_lesson":
				title = "Nominal Rate Lesson"
			elif source_key == "effective_rate_lesson":
				title = "Effective Rate Lesson"
			elif source_key == "continuous_compounding_lesson":
				title = "Continuous Compounding Lesson"
			connection.execute(
				"INSERT OR IGNORE INTO interactive_sources (source_key, title, course, content, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
				(source_key, title, "Engr Econ", content, now, now),
			)
			interactive = connection.execute("SELECT id, title, course, content FROM interactive_sources WHERE source_key = ?", (source_key,)).fetchone()
			if interactive:
				...
				for index, chunk in enumerate(chunks):
					connection.execute(
						...
						(interactive["id"], index, chunk, hashlib.sha256(chunk.encode("utf-8")).hexdigest(), now, now),
					)
...
def list_budget_audit_events(database_path, entry_id=None, wallet_id=None):
	with get_connection(database_path) as connection:
		query = "SELECT budget_audit_events.*, budget_entries.wallet_id, budget_entries.title AS entry_title, budget_entries.type AS entry_type, budget_entries.amount AS entry_amount FROM budget_audit_events LEFT JOIN budget_entries ON budget_entries.id = budget_audit_events.entry_id"
		params = []
		filters = []
		if entry_id:
			filters.append("budget_audit_events.entry_id = ?")
			params.append(entry_id)
		if wallet_id:
			filters.append("budget_entries.wallet_id = ?")
			params.append(wallet_id)
		if filters:
			query += " WHERE " + " AND ".join(filters)
		query += " ORDER BY budget_audit_events.created_at DESC, budget_audit_events.id DESC"
		return [_row_to_dict(row) for row in connection.execute(query, params).fetchall()]
