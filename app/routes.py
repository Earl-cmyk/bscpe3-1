...
from .services.posts.compound_interest import CompoundInterestError, calculate_compound_interest
from .services.posts.continuous_compounding import ContinuousCompoundingError, calculate_continuous_compounding
from .services.posts.discount import DiscountError, calculate_discount
from .services.posts.effective_annual__interest_rate import EffectiveRateError, calculate_effective_rate
from .services.posts.equation_of_value import EquationOfValueError, calculate_equation_of_value
from .services.posts.nominal_interest_rate import NominalRateError, calculate_nominal_rate
...
@main.post("/api/interactive/compound-interest")
def calculate_interactive_compound_interest():
	try:
		return jsonify(result=calculate_compound_interest(request.get_json(silent=True) or {}))
	except CompoundInterestError as error:
		return jsonify(error=str(error)), 400


@main.post("/api/interactive/discount")
def calculate_interactive_discount():
	try:
		return jsonify(result=calculate_discount(request.get_json(silent=True) or {}))
	except DiscountError as error:
		return jsonify(error=str(error)), 400


@main.post("/api/interactive/equation-of-value")
def calculate_interactive_equation_of_value():
	data = request.get_json(silent=True) or {}
	if isinstance(data.get("obligations"), str):
		try:
			data["obligations"] = json.loads(data["obligations"])
		except json.JSONDecodeError:
			return jsonify(error="obligations must be valid JSON"), 400
	if isinstance(data.get("payments"), str):
		try:
			data["payments"] = json.loads(data["payments"])
		except json.JSONDecodeError:
			return jsonify(error="payments must be valid JSON"), 400
	try:
		return jsonify(result=calculate_equation_of_value(data))
	except EquationOfValueError as error:
		return jsonify(error=str(error)), 400


@main.post("/api/interactive/nominal-rate")
def calculate_interactive_nominal_rate():
	try:
		return jsonify(result=calculate_nominal_rate(request.get_json(silent=True) or {}))
	except NominalRateError as error:
		return jsonify(error=str(error)), 400


@main.post("/api/interactive/effective-rate")
def calculate_interactive_effective_rate():
	try:
		return jsonify(result=calculate_effective_rate(request.get_json(silent=True) or {}))
	except EffectiveRateError as error:
		return jsonify(error=str(error)), 400


@main.post("/api/interactive/continuous-compounding")
def calculate_interactive_continuous_compounding():
	try:
		return jsonify(result=calculate_continuous_compounding(request.get_json(silent=True) or {}))
	except ContinuousCompoundingError as error:
		return jsonify(error=str(error)), 400
...
def get_budget():
	...
	return jsonify(entries=entries, balance=float(balance), wallets=list_wallets(current_app.config["DATABASE_PATH"], True), contributors=list_contributors(current_app.config["DATABASE_PATH"], True), deposits=float(deposits), withdrawals=float(withdrawals), audit=list_budget_audit_events(current_app.config["DATABASE_PATH"], wallet_id=wallet_id))
...
def get_budget_audit():
	return jsonify(events=list_budget_audit_events(current_app.config["DATABASE_PATH"], request.args.get("entry_id", type=int), request.args.get("wallet_id", type=int)))
