...
function noteMarkup(note) {
	...
	const attachmentDisplay = attachments.length ? `<div class="note-attachments">${attachments.map((attachment) => {
		...
		if (attachment.type?.startsWith('image/')) return `<figure class="note-media"><button class="media-trigger" type="button" data-media-url="${url}" data-media-name="${name}" data-media-type="${escapeHtml(attachment.type)}"><img src="${url}" alt="${name}" loading="lazy" /></button><figcaption><span>${name}</span><a class="text-link" href="${url}?download=1" aria-label="Download ${name}" title="Download ${name}">&#8595;</a></figcaption></figure>`;
		if (attachment.type?.startsWith('video/')) return `<figure class="note-media"><button class="media-trigger" type="button" data-media-url="${url}" data-media-name="${name}" data-media-type="${escapeHtml(attachment.type)}"><video muted preload="metadata"><source src="${url}" type="${escapeHtml(attachment.type)}" /></video></button><figcaption><span>${name}</span><a class="text-link" href="${url}?download=1" aria-label="Download ${name}" title="Download ${name}">&#8595;</a></figcaption></figure>`;
