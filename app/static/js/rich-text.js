...
function isAsciiArt(text) {
	...
	const hasAlignedWhitespace = / {2,}|\t/.test(text);
	const diagramLines = lines.filter((line) => line.trim()).filter((line) => {
		const punctuation = (line.match(/[|/_\\=+\-()[\]<>:]/g) || []).length;
		return punctuation >= 2 && punctuation / Math.max(line.length, 1) >= 0.15;
	}).length;
	return hasAlignedWhitespace || diagramLines >= 2;
}

function createPlainTextPaste(text) {
	const preformatted = isAsciiArt(text);
	if (!preformatted) return document.createTextNode(text);
...
function looksLikeFormula(text) {
	...
	return /(?:=|\+|\-|\*|\/|\^|_|\\frac|\\sqrt|\\sum|\\int)/.test(value) && /[A-Za-z]/.test(value);
}

function formatFormulaText(text) {
	let html = String(text || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
	html = html.replace(/\\frac\s*\{([^{}]+)\}\s*\{([^{}]+)\}/g, '<span class="math-fraction"><span class="math-num">$1</span><span class="math-bar"></span><span class="math-den">$2</span></span>');
	html = html.replace(/([A-Za-z0-9]+)\s*\^\s*(\([^)]+\)|[A-Za-z0-9]+)/g, '<span class="math-base">$1</span><sup class="math-sup">$2</sup>');
	html = html.replace(/([A-Za-z0-9]+)\s*_\s*([A-Za-z0-9]+)/g, '<span class="math-base">$1</span><sub class="math-sub">$2</sub>');
	html = html.replace(/\n/g, '<br>');
	return html;
}

function createFormulaPaste(text) {
	const wrapper = document.createElement('span');
	wrapper.className = 'math-formula';
	wrapper.innerHTML = formatFormulaText(text);
	return wrapper;
}
...
function createCodeBlock(source, language = '') {
	...
	cell.dataset.pythonCell = 'true';
	const toolbar = document.createElement('div');
	toolbar.className = 'python-cell-toolbar';
	const label = document.createElement('span');
	label.textContent = 'Python';
	const run = document.createElement('button');
	run.type = 'button';
	run.className = 'python-cell-run';
	run.dataset.runPython = 'true';
	run.textContent = 'Run';
	toolbar.append(label, run);
	const pre = document.createElement('pre');
	...
	output.className = 'python-cell-output';
	output.dataset.pythonOutput = 'true';
	output.hidden = true;
	cell.append(toolbar, pre, output);
	return cell;
}

function createPlainTextBlocks(text) {
	...
	const appendPlain = () => {
		...
		const rows = parsePlainTextTable(value);
		fragment.appendChild(rows ? createTableFromRows(rows) : createPlainTextPaste(value));
		plainLines = [];
	};
	lines.forEach((line) => {
		...
		if (codeLines !== null && /^\s*```\s*$/.test(line)) {
		...
		else if (!line.trim()) appendPlain();
		else plainLines.push(line);
	});
	if (codeLines !== null) fragment.appendChild(createCodeBlock(codeLines.join('\n'), codeLanguage));
	appendPlain();
	return fragment;
}
...
function loadPyodideRuntime() {
	if (!pyodidePromise) {
		pyodidePromise = new Promise((resolve, reject) => {
			if (window.loadPyodide) return resolve(window.loadPyodide({ indexURL: 'https://cdn.jsdelivr.net/pyodide/v0.27.2/full/' }));
			const script = document.createElement('script');
			script.src = 'https://cdn.jsdelivr.net/pyodide/v0.27.2/full/pyodide.js';
			script.onload = () => window.loadPyodide({ indexURL: 'https://cdn.jsdelivr.net/pyodide/v0.27.2/full/' }).then(resolve, reject);
...
function setupPythonCells(root = document) {
	root.querySelectorAll('[data-python-cell]').forEach((cell) => {
		if (cell.dataset.pythonReady) return;
		cell.dataset.pythonReady = 'true';
		const button = cell.querySelector('[data-run-python]');
		const source = cell.querySelector('code');
		...
		button?.addEventListener('click', async () => {
			...
			try {
				...
				pyodide.setStdout({ batched: (value) => { output.textContent += value; } });
				output.textContent = '';
				await pyodide.runPythonAsync(source.textContent);
				if (!output.textContent) output.textContent = 'Done.';
			} catch (error) {
...
function createRichTextEditor(editor) {
	...
	editor.addEventListener('paste', (event) => {
		...
		if (looksLikeFormula(plainText) && !html) {
			insertAtSelection(createFormulaPaste(plainText));
			sync();
			return;
		}
		if (html) {
			if (/^\s*```(?:python|py)\b/m.test(plainText)) {
				insertAtSelection(createPlainTextBlocks(plainText));
				sync();
				setupPythonCells(editor);
				return;
			}
			...
			} else if (looksLikeFormula(plainText)) {
				insertAtSelection(createFormulaPaste(plainText));
			} else {
				insertAtSelection(wrapper);
			}
