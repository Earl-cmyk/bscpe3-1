...
function announcementMarkup(announcement) {
  ...
  const attachmentUrl = announcement.attachment_path ? `/uploads/${encodeURIComponent(announcement.attachment_path)}` : '';
  const attachmentName = escapeHtml(announcement.attachment_name || announcement.attachment_path || 'Attachment');
  const attachment = attachmentUrl ? (announcement.attachment_type?.startsWith('image/') ? `<button class="announcement-attachment" data-media-url="${attachmentUrl}" data-media-name="${attachmentName}" data-media-type="${escapeHtml(announcement.attachment_type)}" type="button"><img src="${attachmentUrl}" alt="${attachmentName}" loading="lazy" /><span>${attachmentName}</span></button>` : announcement.attachment_type?.startsWith('video/') ? `<button class="announcement-attachment announcement-video" data-media-url="${attachmentUrl}" data-media-name="${attachmentName}" data-media-type="${escapeHtml(announcement.attachment_type)}" type="button"><span aria-hidden="true">&#9654;</span><span>${attachmentName}</span></button>` : `<a class="text-link icon-action" href="${attachmentUrl}" target="_blank" rel="noreferrer" aria-label="Open attachment" title="Open attachment">&#128206;</a>`) : '';
  ...
  return `<article class="announcement-item"><div class="announcement-meta"><span>${new Date(announcement.created_at).toLocaleDateString()}</span>${link}${attachment}</div><h3>${escapeHtml(announcement.title)}</h3><div class="rich-content">${announcement.body || ''}</div>${options ? `<div class="poll-options">${options}</div>` : ''}</article>`;
}
